...
function AdminUserTable({ users, userUsernameSearch, handleInputChange, handleDeleteUser, handleEnableUser }) {
  let userList
  if (users.length === 0) {
    userList = (
    ...
    userList = users
      .filter((user) => {
        return user.username.toLowerCase().includes(userUsernameSearch.toLowerCase())
      })
      .map(user => {
        return (
